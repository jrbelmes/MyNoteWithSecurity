...
import ProfileModal from './profile';  // Make sure this import is correct
...
const sidebarVariants = {
  ...
  mobile: {
    width: '75%', // Reduced from 85% to give more space
    transition: { duration: 0.3, type: 'tween' }
  }
};
    ...
    <SidebarContext.Provider value={contextValue}>
      <div className="flex h-screen">
        ...
        <div className="flex w-full h-full relative">
          <motion.aside
            initial="closed"
            animate={isSidebarOpen ? (isMobile ? "mobile" : "open") : "closed"}
            variants={sidebarVariants}
            className={`
              fixed md:relative h-full bg-white dark:bg-gray-800 
              text-gray-700 dark:text-gray-200 shadow-xl flex flex-col 
              ${isMobile ? 'z-40 left-0' : 'z-30'}
              ${isSidebarOpen && isMobile ? 'max-w-[75%]' : ''}
          ...
          >
            ...
            <div className="flex-grow overflow-y-auto">
              <nav className="mt-5 px-2">
                ...
                <SidebarItem icon={FaCar} text="View Task" link="/viewTask" active={activeItem === '/viewTask'} />
              </nav>
            </div>
...
const SidebarItem = React.memo(({ icon: Icon, text, link, active }) => {
  ...
  return (
    ...
    >
      <Icon className={`
        text-xl ${active ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-gray-500'}
        ${!isSidebarOpen ? 'mr-0' : ''}
      `} />
      {isSidebarOpen && (
        <motion.span
