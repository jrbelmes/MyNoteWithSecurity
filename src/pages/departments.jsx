...
const Departments = () => {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch('http://localhost/coc/gsd/delete_master.php', {
        ...
        body: new URLSearchParams({ operation: 'deleteDepartment', departments_id: selectedDepartmentId }),
      });
  ...
  const handleSave = async () => {
    try {
      ...
      const data = await response.json();
      if (data.status === 'success') {
        setDepartments(departments.map(dept => dept.departments_id === formData.id ? { ...dept, departments_name: formData.name } : dept));
